...
def do_approve(request, app):
    ...
    if created:
        ...
        temp_password = '[user exists - use Reset Password]'
    ...
    def approve_selected(self, request, queryset):
        for app in queryset.filter(status='pending'):
            try:
                ...
                messages.error(request, 'Error approving ' + app.full_name + ': ' + str(e))
...
class TherapistProfileAdmin(admin.ModelAdmin):
    list_display = ['get_name', 'get_email', 'status', 'rating', 'is_available', 'whatsapp_number']
...
class PatientAssignmentAdmin(admin.ModelAdmin):
    list_display = ['get_therapist', 'get_patient', 'status', 'module_id', 'assigned_at']
    list_filter = ['status']

    def get_therapist(self, obj):
        try:
            return obj.therapist.user.name or obj.therapist.user.email
        except Exception:
            return '-'
...
class HandoffEventAdmin(admin.ModelAdmin):
    list_display = ['get_patient', 'get_therapist', 'reason', 'accepted', 'created_at']
    ...
    def get_therapist(self, obj):
        try:
            return obj.therapist.user.name if obj.therapist else '-'
        except Exception:
            return '-'
    get_therapist.short_description = 'Therapist'
